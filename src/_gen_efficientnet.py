...
from src.config import layer_config_kwargs, is_scriptable
from src.conv2d_layers import select_conv2d
from src.helpers import load_pretrained
from src._efficientnet_builder import *
...
class GenEfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, 
                 num_features=1280, stem_size=32, fix_stem=False,
                 channel_multiplier=1.0, channel_divisor=8, channel_min=None,
                 pad_type='', act_layer=nn.ReLU, drop_rate=0., drop_connect_rate=0.,
                 se_kwargs=None, norm_layer=nn.BatchNorm2d, norm_kwargs=None,
                 weight_init='goog',positional_embedings=''):
        super(GenEfficientNet, self).__init__()
        self.drop_rate = drop_rate
        self.positional_embedings = positional_embedings
        ...
        if 'sincos' in self.positional_embedings:
            if 'concat' in self.positional_embedings:
                self.classifier = nn.Linear(2*num_features, num_classes)
            elif 'emb+' in self.positional_embedings:
                self.embedings = nn.Linear(num_features,num_features)
                self.classifier = nn.Linear(num_features, num_classes)
            else:
                raise Exception('Invalid positional embedings - {}'.format(self.positional_embedings))
        # elif self.positional_embedings == 'coords':
        #     if '1hot' in self.positional_embedings:
        #         pass
        #     elif 'vals' in self.positional_embedings:
        #         pass
        #     else:
        #         raise Exception('Invalid positional embedings - {}'.format(self.positional_embedings))
        elif not self.positional_embedings:
            self.classifier = nn.Linear(num_features, num_classes)
        else:
            raise Exception('Invalid positional embedings - {}'.format(self.positional_embedings))
        ...
        # self.foo = nn.Linear(num_features,num_features)
    ...
    def forward(self, x,y=None):
        if (self.positional_embedings) and (y is None):
            raise Exception("When truned on, possitional embedings must be provided on model.forward")
        x = self.features(x)
...
def _gen_efficientnet(variant, channel_multiplier=1.0, 
                      depth_multiplier=1.0, pretrained=False, **kwargs):
